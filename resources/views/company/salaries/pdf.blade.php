        ...
        <table class="header-content">
            <tr>
                <td class="header-text">
                    <h1>{{ $company->name }}</h1>
                    <p style="margin-top: 5px;">Salary Payment Report</p>
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th width="20%" class="text-right">Gross Total</th>
            </tr>
        </thead>
        <tbody>
            @php $totalBase = 0; $totalBonus = 0; $totalGross = 0; @endphp
            @foreach($salaries as $salary)
                @php 
                    $gross = $salary->amount + $salary->bonus;
                    $totalBase += $salary->amount;
                ...
                <tr>
                    ...
                    <td>
                        <div class="font-bold">{{ $salary->teamMember->name }}</div>
                        <div style="font-size: 9px; color: #777;">{{ $salary->teamMember->designation }}</div>
                    </td>
                    <td class="text-right">${{ number_format($salary->amount, 2) }}</td>
                    <td class="text-right bonus-amount">
                        {{ $salary->bonus > 0 ? '$' . number_format($salary->bonus, 2) : '-' }}
                    </td>
                    <td class="text-right font-bold">${{ number_format($gross, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr class="font-bold total-row">
                <td colspan="2" class="text-right" style="border-right: none;">GRAND TOTAL:</td>
                <td class="text-right" style="border-left: none;">${{ number_format($totalBase, 2) }}</td>
